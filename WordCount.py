...
class Pref:
    ...
    def load(self):
        ...
        Pref.wrdRx = re.compile(s.get('word_regexp', "^[^\w]?`*\w+[^\w]*$"),
                                re.U)
        ...
        Pref.page_count_mode_count_words = s.get('page_count_mode_count_words',
                                                 True)
        Pref.char_ignore_whitespace = s.get('char_ignore_whitespace', True)
        Pref.readtime_wpm = s.get('readtime_wpm', 200)
        Pref.whitelist = [
            x.lower() for x in s.get('allowed_syntaxes', []) or []
        ]
        Pref.blacklist = [
            x.lower() for x in s.get('rejected_syntaxes', []) or []
        ]
...
class WordCount(sublime_plugin.EventListener):
    ...
    def should_run_with_syntax(self, view):
        ...
        syntax = basename(syntax).split(
            '.')[0].lower() if syntax != None else "plain text"
    ...
    def run(self, asap=False):
        if not Pref.view:
            ...
            if vs.get(
                    'is_widget'
            ) or not ws:  # (if not ws)WTF, happens when closing a view
                self.guess_view()
            else:
                if (ws['modified'] or ws['selection']) and (
                        Pref.running == False
                        or asap) and self.should_run_with_syntax(view):
                    sel = view.sel()
                    if sel:
                        if len(sel) == 1 and sel[0].empty():
                            if not Pref.enable_live_count or view.size(
                            ) > 10485760:
                                view.erase_status('WordCount')
                                ...
                                WordCountThread(
                                    ...
                                    view.substr(view.line(
                                        view.sel()[0].end())), False).start()
                            else:
                                # print('running from cache:'+str(view.change_count()))
                                view.set_status(
                                    'WordCount',
                                    self.makePlural('Word', ws['count']))
                        else:
                            try:
                                WordCountThread(
                                    view, [
                                        view.substr(
                                            sublime.Region(s.begin(), s.end()))
                                        for s in sel
                                    ],
                                    view.substr(view.line(
                                        view.sel()[0].end())), True).start()
                            except:
                                pass
                        ws['modified'] = False
                        ws['selection'] = False
    ...
    def display(self, view, on_selection, word_count, char_count,
                word_count_line, char_count_line):
        ...
        if Pref.enable_count_pages and word_count > 0:
            if not Pref.page_count_mode_count_words or Pref.words_per_page < 1:
                ...
                rows_per_page = (view.rowcol(visible.end())[0]) - (view.rowcol(
                    visible.begin())[0])
                pages = ceil(
                    (view.rowcol(view.size() - 1)[0] + 1) / rows_per_page)
                ...
                current_line = view.rowcol(view.sel()[0].begin())[0] + 1
                current_page = ceil((current_line / Pref.words_per_page) /
                                    (rows / Pref.words_per_page))

            if pages > 1:
                if current_page != 0:
                    status.append('Page ' + str(current_page) + '/' +
                                  str(pages))
                else:
                    status.append('Page ' + str(current_page) + '/' +
                                  str(pages))
...
class WordCountThread(threading.Thread):
    ...
    def run(self):
        ...
        if Pref.enable_count_chars:
            if Pref.char_ignore_whitespace:
                self.char_count = sum(
                    [len(''.join(region.split())) for region in self.content])
            else:
    ...
    def on_done(self):
        try:
            WordCount().display(self.view, self.on_selection, self.word_count,
                                self.char_count, self.word_count_line,
                                self.chars_in_line)
        except:
            pass
        Pref.running = False
    ...
    def count(self, content):
        ...
        if splitRx:
            words = len([
                1 for x in splitRx(content)
                if False == x.isdigit() and wrdRx(x)
            ])
        else:
            words = len([
                1 for x in content.replace("'", '').replace('—', ' ').replace(
                    '–', ' ').replace('-', ' ').split()
                if False == x.isdigit() and wrdRx(x)
            ])
...
def word_count_loop():
    ...
    while True:
        # sleep time is adaptive, if takes more than 0.4 to calculate the word
        # count sleep_time becomes elapsed_time*3
